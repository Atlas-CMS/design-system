...
const __Typography = forwardRef(
  ({ children, ...props }: TypographyProps, ref) => {
    const theme = useTheme();
    // console.log({ children, theme, props });
    let { className, ...text_props } = mapTypographyPropsToMantine(
      props,
      theme
    );
    ...
    return (
      <Text
        ref={ref}
        className={clsx("atlas-Typography-sds", className)}
        {...text_props}
      >
        {children}
      </Text>
...
const Typography = forwardRef<
  HTMLDivElement,
  React.ComponentProps<typeof _Typography>
>(
  (
    { as = "p", ...props }: PolymorphEscapeHatch<TypographyProps & TextProps>,
    ref
