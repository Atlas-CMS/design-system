...
export type BoxProps<TElement extends keyof JSX.IntrinsicElements = 'div'> =
...
export const Box = styled.div.withConfig<BoxProps>({
  shouldForwardProp: (prop, defPropValFN) =>
    !transientProps[prop as keyof BoxProps] && defPropValFN(prop),
})`
  // Font
  ...
  background: ${({ theme, background }) =>
    extractStyleFromTheme(theme.colors, background, background)};
  ...
  // Responsive hiding
  ${({ theme, hiddenS }) =>
    hiddenS ? `${theme.mediaQueries.tablet} { display: none; }` : undefined}
  ${({ theme, hiddenXS }) =>
    hiddenXS ? `${theme.mediaQueries.mobile} { display: none; }` : undefined}
  

  // Borders
  border-radius: ${({ theme, hasRadius, borderRadius }) =>
    hasRadius ? theme.paperBorderRadius : borderRadius};
  border-style: ${({ borderStyle }) => borderStyle};
  border-width: ${({ borderWidth }) => borderWidth};
  border-color: ${({ borderColor, theme }) =>
    extractStyleFromTheme(theme.colors, borderColor, undefined)};
  ...
  max-height: ${({ maxHeight, theme }) =>
    extractStyleFromTheme(theme.spaces, maxHeight, maxHeight)};
  min-height: ${({ minHeight, theme }) =>
    extractStyleFromTheme(theme.spaces, minHeight, minHeight)};
  ...
  line-height: ${({ theme, lineHeight }) =>
    extractStyleFromTheme(theme.lineHeights, lineHeight, lineHeight)};
